...
public class FlightServiceTest {
  ...
  static List<User> parse(String filename) throws IOException, SQLException {
    ...
    if (cmds.size() > 0 || r.length() > 0 || results.size() > 0) {
      throw new IllegalArgumentException(String.format(
          "Input file %s is malformatted, extra information found #commands=%s, len(result)=%s, #results=%s",
  ...
  public static void setup() {
    ...
    try {
      ...
      if (tableSuffix != null) {
        ...
        System.out.println("... fully resetting database (ie, dropping everything except domain tables)");
      }
